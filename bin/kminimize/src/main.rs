...
fn disable(args: DisableArgs) -> anyhow::Result<()> {
    ...
    let kbuild = kbuild::KbuildMaps::load(&args.kernel_src)
    ...
    let kconfig = kconfig::KconfigGraph::load(&args.kernel_src)
    ...
    if !forced_on.is_empty() {
        // List idle-category filtered symbols individually (few of them).
    ...
    // Report loaded-but-idle with full per-symbol list.
    ...
    // Report never-loaded as a count-only summary.
...
fn write_dotconfig(config_path: &Path, to_disable: &HashSet<String>) -> anyhow::Result<()> {
    ...
    for line in original.lines() {
        // Match CONFIG_FOO=value lines where CONFIG_FOO is in to_disable.
        let sym = line
            .split_once('=')
    ...
    // Symbols not found anywhere in the file: append at the end.
